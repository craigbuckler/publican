...
export function mdHTML(str, mdOpts = mdConfig) {
  ...
  // initial code lines
  str = str
    .replace(/<p>(<nav-heading.*?<\/nav-heading>)<\/p>/ig, '$1')  // remove <nav-heading> paragraphs
    .replace(/(<span class="token[^>]*>)`/ig, '$1&#96;')          // escape code backticks
    .replace(/(<span class="token[^>]*>&#96;<\/span>)+/ig, '$1')  // replace multiple backticks
    .replace(/(<span class="token[^>]*>)\$\{/ig, '$1&#36;{')      // escape code ${ characters
    .replace(/(<pre class="language-.*?)(\s*<span class="token.+?)\n/gi, '$1<span class="line">$2</span>\n'); // add lines
  ...
  do {
    oc = str;
    str = str
      .replace(/\n([\s\w]*?<span class="token.*?)\n/gi, '\n<span class="line">$1</span>\n');
  } while (str !== oc);

  // add whitespace lines
  str = str
    .replace(/\n{2,}(<span class="line">)/gi, '\n<span class="line"></span>\n$1');
...
const navHeadingTag = '</nav-heading>';
export function navHeading(str, haOpt) {
  ...
  return str.replaceAll(navHeadingTag, contentsList + navHeadingTag);
